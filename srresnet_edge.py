...
def ResidualBlock(x, kernel_size, filter_size):
...
def Upsample2xBlock(x, kernel_size, filter_size):
	weights = {
...
def foward(x_in, x_edge, b_block=8):
...
def rect_loss(y_HR_hat, y_predict):
	return tf.reduce_mean(tf.square(y_HR_hat - y_predict))


def edge_loss(y_edge_HR_hat, y_predict):
	return tf.reduce_mean(tf.square(y_edge_HR_hat - y_predict))

def total_loss(rect_loss, edge_loss):
	return rect_loss + edge_loss
