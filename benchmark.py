...
class Benchmark:
    ...
    def eval(self, sess, x_H, x_edge, log_path=None, iteration=0):
        pred = []
        
        for i, x in enumerate(self.images_lr):
            x = x/255.0

            lr_edge = self.image_lr_edge[i]
            lr_edge = np.expand_dims(lr_edge,axis=-1)

            output_rect,output_edge = sess.run([x_H, x_edge], feed_dict={'LR_image:0': x[np.newaxis],'LR_edge:0':lr_edge[np.newaxis]})
            
            print("__DEBUG__out",np.squeeze(output_rect, axis=0).shape)
            pred.append(self.deprocess(np.squeeze(output_rect, axis=0)))
                # save images
        if log_path:
            self.save_images(pred, log_path, iteration)
        return self.test_images(self.images_hr, pred)
